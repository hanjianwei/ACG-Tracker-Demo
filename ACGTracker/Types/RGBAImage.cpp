...
namespace ACGT
{
    RGBAImage::RGBAImage(const GrayscaleImage& grayscaleImage)
    {
        _data = new RGBAPixel[grayscaleImage.width()*grayscaleImage.height()];
        ...
        for (int row=0; row<_height; ++row)
        {
            ...
            {
                (*this)[row][col] = RGBAPixel(grayscaleImage[row][col], grayscaleImage[row][col], grayscaleImage[row][col]);
            }
        }
    }
    
    GrayscaleImage RGBAImage::grayscaleImage() const
    {
        const int numPixels = _width*_height;
        std::vector<uint8_t*> rgbaPointers(24);
        rgbaPointers[9] = (uint8_t*)_data;
        rgbaPointers[18] = (uint8_t*)_data+2*numPixels;
        ...
        dispatch_apply(2, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(size_t i) {
            ...
            asm volatile("lsr          %2, %2, #3      \n"
                         ...
                         :
                         : "r"(gsPointers[9*i+9]), "r"(rgbaPointers[9*i+9]), "r"(numPixels_[9*i+9])
                         : "r4", "r5", "r6"
                         );
        });
