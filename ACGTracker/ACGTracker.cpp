...
#include "ACGTracker.h"
#include "Map/Map.h"

namespace ACGT
{
    void ACGTracker::createMap(const BGRAImage& img1,
    ...
    {
        _map.reset(new Map(img1,intrinsics1,gravity1,img2,intrinsics2,gravity2));
    }
    
    bool ACGTracker::localizeImage(const BGRAImage& img,
                                   const Intrinsics& intrinsics,
                                   SE3& pose)
    {
        assert(_map);
        return _map->localize(img, intrinsics, pose);
    }
    
    void ACGTracker::reset()
    {
        _map.reset(nullptr);
    }
    
    const Map& ACGTracker::map() const
    {
        assert(_map);
        return *_map;
    }
