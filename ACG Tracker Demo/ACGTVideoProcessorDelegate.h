...
@protocol ACGTVideoProcessorDelegate <NSObject>

@required
- (void)imageDataReadyForDisplay:(ACGTARViewImageData&)imageData;	// This method is always called on the main thread.
- (void)imageDataReadyForDisplay:(ACGTARViewImageData&)imageData
                      insetImage:(ACGTARViewImageData&)inset
                         matches:(std::vector<ACGTARViewColoredVertex>&)matches;	// This method is always called on the main thread.
