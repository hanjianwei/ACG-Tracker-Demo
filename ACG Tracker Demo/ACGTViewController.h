...
typedef struct ACGTCamPositionData
{
...
static constexpr unsigned int numCamPositionsToDisplay = 100;

@interface ACGTViewController : UIViewController <ACGTVideoProcessorDelegate, GLKViewDelegate>
{
    UIToolbar *_toolbar;
    
    ACGTVideoProcessor *_videoProcessor;
    
    ACGTARView *_arView;
    
    GLKView *_camPositionView;
    GLuint _vertexBuffer;
    GLuint _camPositionProgram;
    
    std::vector<ACGTCamPositionData> _camPositions;
    int _camPositionCounter;
    float _mapScale;
    
    UILabel *_frameRateLabel;
    std::list<float> _lastFrameProcessingTimes;
}
